...
@Repository
class WindowRepositoryImpl(private val jdbcTemplate: JdbcTemplate) : WindowRepository {
    private val logger = LoggerFactory.getLogger(WindowRepositoryImpl::class.java)

    @Transactional
    override suspend fun save(window: ADWindow): Either<WindowError, ADWindow> =
            try {
                logger.debug("Saving window: ${window.id}")

                jdbcTemplate.update(
                        """
            INSERT INTO ad_window (id, name, display_name, description, is_active, is_sotrx, window_type)
            VALUES (?, ?, ?, ?, ?, ?, ?)
    ...
    private fun saveTabs(window: ADWindow) {
        ...
        window.tabs.forEach { tab ->
            ...
            jdbcTemplate.update(
                    """
                INSERT INTO ad_tab (id, window_id, name, display_name, description, table_id, sequence)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            """,
                    tab.id,
                    window.id,
        ...
        tab.fields.forEach { field ->
            jdbcTemplate.update(
                ...
                INSERT INTO ad_field (
                    id, tab_id, column_name, display_name, description,
                    is_displayed, is_readonly, is_mandatory, sequence,
                    display_logic, default_value, validation_rule
                )
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
                    field.id,
    ...
    @Transactional
    override suspend fun delete(id: UUID): Either<WindowError, Unit> =
            try {
                val deleted = jdbcTemplate.update("DELETE FROM ad_window WHERE id = ?", id)
                if (deleted > 0) Unit.right() else WindowError.NotFound(id).left()
            } catch (e: Exception) {
                logger.error("Failed to delete window: $id", e)
                WindowError.ValidationFailed("Delete failed: ${e.message}").left()
            }

    override suspend fun findById(id: UUID): Either<WindowError, ADWindow?> =
            try {
                val window =
                        jdbcTemplate.queryForObject(
                                """
            SELECT w.*, u1.username as created_by, u2.username as updated_by, w.version
            FROM ad_window w
                                    ...
                                    ADWindow(
                                            ...
                                            displayName =
                                                    DisplayName.create(rs.getString("display_name"))
                                                            .orNull()!!,
                                            name =
                                                    WindowName.create(rs.getString("name"))
                                                            .orNull()!!,
                                            description =
    ...
    override suspend fun findByName(name: WindowName): Either<WindowError, ADWindow?> =
            try {
                val window =
                        jdbcTemplate.queryForObject(
                                """
    ...
    private fun findTabsByWindowId(windowId: UUID): List<ADTab> {
        return jdbcTemplate.query(
                """
            SELECT t.*, u1.username as created_by, u2.username as updated_by, t.version
            FROM ad_tab t
    ...
    override suspend fun search(query: String, pageSize: Int, page: Int): Flow<ADWindow> = flow {
        val sql =
            ...
            LEFT JOIN ad_user u1 ON w.created_by = u1.id
            LEFT JOIN ad_user u2 ON w.updated_by = u2.id
            WHERE LOWER(w.name) LIKE LOWER(?) OR LOWER(w.display_name) LIKE LOWER(?)
            ORDER BY w.name
            LIMIT ? OFFSET ?
        """
